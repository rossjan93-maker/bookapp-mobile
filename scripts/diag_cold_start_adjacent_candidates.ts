...
// CLI flags (Phase A.1): --sort=<relevance|editions>, --min-year=N, --top-n=N.
function parseFlag(name: string, fallback: string): string {
  const pre = `--${name}=`;
  const hit = process.argv.find(a => a.startsWith(pre));
  return hit ? hit.slice(pre.length) : fallback;
}
const SORT_MODE: 'relevance' | 'editions' =
  (parseFlag('sort', 'relevance') === 'editions' ? 'editions' : 'relevance');
function parseIntFlag(name: string, raw: string, fallback: number, minAllowed: number): number {
  const n = parseInt(raw, 10);
  if (!Number.isFinite(n) || n < minAllowed) {
    console.warn(`[diag] invalid --${name}=${JSON.stringify(raw)} (must be integer >= ${minAllowed}); using default ${fallback}`);
    return fallback;
  }
  return n;
}
const MIN_YEAR         = parseIntFlag('min-year', parseFlag('min-year', '1980'), 1980, 0);
const TOP_N            = parseIntFlag('top-n',    parseFlag('top-n', '8'),       8,    1);
const POLITENESS_MS    = 350;
const OUT_DIR          = '.local';
const OUT_FILE         = `cold_start_adjacent_evidence_report_${SORT_MODE}_${MIN_YEAR}.md`;
...
async function fetchAnchorTopN(anchor: string, n: number): Promise<OLDoc[]> {
  ...
  const sortParam = SORT_MODE === 'editions' ? '&sort=editions' : '';
  const url =
    `https://openlibrary.org/search.json?subject=${subj}` +
    `&limit=${n * 3}` +                                       // over-fetch; we may filter by year client-side
  ...
  const filtered = MIN_YEAR > 0
    ? docs.filter(d => typeof d.first_publish_year === 'number' && d.first_publish_year >= MIN_YEAR)
    : docs;
...
function md(rows: Classified[]): string {
  ...
  lines.push('Anchors are the Phase A Mystery + Thriller `ADJACENT_RETRIEVAL_ANCHORS`. ' +
    `Top-${TOP_N} OL candidates per anchor (sort=${SORT_MODE}` +
    (MIN_YEAR > 0 ? `, first_publish_year >= ${MIN_YEAR}` : '') + `). ` +
  ...
  // Phase A.1: anchors below useful-threshold alert (post-year-filter shrinkage).
  const byAnchorCounts = new Map<string, number>();
  for (const r of rows) byAnchorCounts.set(r.anchor, (byAnchorCounts.get(r.anchor) ?? 0) + 1);
  const thin = [...byAnchorCounts.entries()].filter(([, n]) => n < 4);
  if (thin.length > 0) {
    lines.push(`- ⚠️  Anchors with < 4 candidates after year filter (MIN_YEAR=${MIN_YEAR}): ` +
      thin.map(([a, n]) => `\`${a}\` (${n})`).join(', '));
  }
  lines.push('');
  lines.push(`_Capture settings: sort=${SORT_MODE}, min_year=${MIN_YEAR}, top_n=${TOP_N}._`);
  return lines.join('\n') + '\n';
}
