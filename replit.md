...
- **Pending migrations** (apply via Supabase dashboard SQL editor):
    ...
    - `20260509000000_p1_catalog_hardening_clarity.sql`
...
- **Catalog write protection (fail-loud):** `_books_protect_identity_columns()` is a `BEFORE UPDATE` trigger that **raises** (SQLSTATE `42501`, message prefix `CATALOG_PROTECTED:`) when a non-service-role caller attempts to modify a protected column. Field classification — *user-mutable* (untouched): `page_count`, `subjects`, `content_warnings`, `cover_source`, `metadata_confidence`, `isbn`, `isbn13`, `additional_authors`, `published_year`. *Immutable post-insert*: `title`, `author`. *Fill-if-empty (NULL → non-NULL only)*: `external_id`, `cover_url`, `description`. Service-role writes bypass (`auth.uid() IS NULL`). Migration `20260509000000` replaced the prior silent-revert behaviour from `20260508000000` because silent reversion hid bugs (e.g. the dormant cover-upgrade branch in `lib/metadataRepair.ts` was already silently broken). Companion identity-column triggers protect `book_source_links.(book_id, source)` and `book_enrichment_cache.external_id` — the natural ON CONFLICT keys — from drift. The cover-upgrade branch in `lib/metadataRepair.ts` now performs its overwrite in a separate `await` so the expected 403 doesn't poison the legitimate fill-empty patch for description/subjects/page_count built in the same loop iteration; the upgrade itself is filed as a follow-up (would need a service-role RPC).
