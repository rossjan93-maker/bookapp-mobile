...
- **Social Sign-In:** Integrates Google and Apple sign-in with user-friendly error messages. Google OAuth: `lib/socialAuth.ts` is the sole owner of the PKCE `exchangeCodeForSession` call while a WebBrowser flow is in flight — `app/auth/callback.tsx` checks `isOAuthInFlight()` and skips its own exchange to avoid the duplicate-exchange "invalid grant" race. The shared OAuth helper also listens on `expo-linking` for the duration of the flow so an Android Custom Tabs `dismiss` result still recovers the redirect URL. See `docs/google-signin.md` for the working Supabase / Google Cloud configuration and a 5-step troubleshooting checklist.
