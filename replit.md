...
| **P4C.1 · limited ranking influence for intent / semantic-fit contributions** | ✅ **product accepted — 2026-05-16** | Signed values for the seven P4C kinds (tone/pace/complexity/current_intent/series_continuation/avoidance_conflict/not_right_now_risk) under per-kind ±0.20 / stack ±0.30 caps with stated-taste floor protection (`clampP4IntentStack`). Composer suppression unchanged — reasons[] copy byte-identical. `recValidity.VERSION` = `rcv5`. Live smoke (quick-taste: Thriller&Mystery + Romance, escape, darker, fast, avoid Horror): PASS WITH NOTE — top 4 cards directionally shaped by fast/dark/escape intent (Sometimes I Lie, Verity, The Perfect Marriage, Never Lie); no horror visible; explanations conservative (no intent claim leakage, no snake_case, no unsupported tone/pace copy); no layout regression. Validators green: `validate_p4c_limited_ranking` (13 sections incl. integration-arithmetic invariant), `validate_intent_contribution`, `validate_tone_pace_fit`, `validate_series_continuation`. |
