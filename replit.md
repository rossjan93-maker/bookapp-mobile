...
| **P3A · contribution-grounded ranking + explanation faithfulness** | ✅ **shipped — 2026-05-16** · ⏳ **product acceptance pending live re-smoke** | Composer-backed `book.reasons[]` is the production default. `recValidity.VERSION` = **`rcv4`** (bumped twice on 2026-05-16 — detectGenre fix `rcv2→rcv3`, then display-label fix `rcv3→rcv4`). Live-smoke status: **A = PASS WITH NOTE**, **B = PASS WITH NOTE** (re-smoke 2026-05-16 after display-label fix — humanised `thriller & mystery` label rendered, no raw snake_case key, no false nonfiction reason, no empty reason, no library-history overclaiming; strategic note that pace/tone/intent inputs don't yet flow into reasons is tracked under Current Intent Layer, NOT a P3A blocker), **C = PASS WITH NOTE**, **D = pending capture**. Full P3A history (foundation, P3A-6-C default-on flip, Scenario B detectGenre fix, Scenario B display-label fix) in `docs/recently_shipped.md`. |
...
## P3A current blockers

- **Live re-smoke A / B / C / D outstanding** — see Phase status row above. P3A flips from "shipped" to "product accepted" only when all four are clean.
- **P3 readiness D1–D5 decisions** are locked + the foundation batch shipped; archived to `docs/recently_shipped.md` ("P3 readiness — D1–D5 decisions").
