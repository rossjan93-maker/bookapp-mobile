...
| `lib/recPayloadCache.ts` | **Persistent rec payload cache** — AsyncStorage-backed; key: `readstack_rec_v1_{userId}`; TTL 2h; `saveRecPayload()` / `loadRecPayload()` / `clearRecPayload()`; `computeRecFingerprint(signalCount, recMode, isFreePreview, intentTag)` → `v1:<n>:<mode>:<fp\|nfp>:<intent>`; `PersistedRecPayload` includes `fingerprint` field; `loadRecPayload` rejects on TTL expiry, corrupt JSON, missing `recs` array, or empty payload — but NOT on fingerprint mismatch (Phase 2 background always corrects); logs fingerprint + mode + signal at every load/save. **Acted-on ID store** (`readstack_rec_acted_v1_{userId}`): `addActedOnIds()` / `loadActedOnIds()` / `clearActedOnIds()` — append-only Set of book IDs acted on (saved/dismissed/more-like-this), read at cache-restore time to filter stale cards before Phase 1 completes |
| `lib/recPrewarm.ts` | **Background recommendation prewarm** — `triggerRecPrewarm(supabase, userId)` (fire-and-forget, returns void synchronously); `_prewarmInFlight` concurrency guard; fingerprint dedup: skips if `computeRecFingerprint(profile)` matches `_lastPrewarmFingerprint` AND `Date.now()-_lastPrewarmAt < PREWARM_COOLDOWN_MS (5 min)`; updates `_lastPrewarmFingerprint/_lastPrewarmAt` after successful save; IIFE has `.catch(() => {})` belt-and-suspenders on outer promise; skips if signal < 3; saves fingerprint in payload |
| `lib/recFeedback.ts` | Recommendation feedback helpers: `persistFeedback()`, `loadFeedbackContext()`, `FeedbackContext` — `dismissedIds` (exclusion), `savedIds` (cache reconciliation), `genreBoosts` (scoring step 4). `loadFeedbackContext` now loads `saved` feedback type to populate `savedIds` |
