...
| **P3A · contribution-grounded ranking + explanation faithfulness** | ✅ **shipped — 2026-05-16 (P3A-6-C default-on flip + Scenario B detectGenre fix)** | composer-backed `book.reasons[]` is the production default (`COMPOSER_REASONS_PROJECTION_ENABLED=true`); contribution-based `explanation_quality` classifier authoritative under the flag (`lib/explanations/contributionQuality.ts`); `recValidity.VERSION` bumped `rcv1` → `rcv2` → **`rcv3`** to invalidate first the pre-flip persisted reasons (P3A-6-C) and then the pre-detectGenre-fix scored payloads (Scenario B fix — fiction books carrying false `stated_favorite:nonfiction` audit flags). Legacy `classifyExplanationQuality` + legacy reasons builder retained as non-empty fallback for one release. Dual-path replay validator `scripts/validate_explanation_default_flip.ts` proves no ranking shift end-to-end. **Scenario B blocker fixed in `lib/bookTraits.ts::detectGenre`**: substring `corpus.includes()` → word-boundary regex; broad nonfiction signals (`science`, `history`, `psychology`, `philosophy`, `technology`, `sociology`) pruned; bucket order reshuffled to memoir_bio → nonfiction → fiction buckets → literary (justified deviation from spec's nonfiction-last recommendation — closes mixed-tag true-nonfiction regression, safe because the pruned signal list no longer carries any fiction-side substring leak). New `scripts/validate_detect_genre.ts` pins the regression (10 detector cases + Scenario B cross-stage detector→stated-taste fixture). Live re-smoke of Scenarios A/B/C/D pending (see `docs/recently_shipped.md`). |
| P4 · semantic intelligence foundation | ⏳ future | hard-avoid UI + storage lands here; first opportunity to retire the legacy reasons builder once P3A-6-C live smoke is clean |
...
## P3 readiness — D1–D5 approved (2026-05-16)

Five decisions locked before P3A implementation begins. **P3A unblocked 2026-05-16** — both prerequisites satisfied: (a) Phase 1 closeout captured in docs, and (b) migration `20260516000000_import_rows_user_book_id_set_null.sql` applied/verified in Supabase. P3A-1 foundation batch (typed contribution model + multi-source retrieval provenance + `fetchOLByAuthor` reason fix + first validator) shipped 2026-05-16; see `docs/recently_shipped.md`.

- **D1 — Multi-source retrieval provenance.** Approved. Each candidate carries `_retrieval_reason[]` (array, not single string), populated as branches contribute. Required for truthful contribution attribution: a book retrieved by both `stated_genre:mystery` AND `revealed_lane:literary_thriller` should be attributable to both, and the AND-gate / contribution accounting must read the union. Today's single-string `_retrieval_reason` becomes the "first-seen" entry of the array for backward compatibility; no behaviour change at the existing AND-gate, additive only.
- **D2 — Fix `fetchOLByAuthor` hardcoded reason label.** Approved. The function currently emits a literal `'author_anchor:'` reason for every book regardless of which author was queried. P3 needs accurate per-author provenance for contribution evidence. Smallest correct fix: thread the resolved author key into the call site and emit `author_anchor:<authorKey>`. No retrieval policy change.
- **D3 — Slate diversity guard in its own file.** Approved. New `lib/composition/slateDiversity.ts` (pure function: `(composed: ScoredBook[], topN: number, contributions: ContributionMap) → ScoredBook[]`). Keeps the kind-frequency rule (reject 3rd instance of same dominant `kind` in top-N unless no alternative) testable in isolation and lets P5 plug in a richer diversity model without touching `recommender.ts` or `statedReservation.ts`.
- **D4 — Per-kind display floors live in `lib/scoring/contributions.ts`, NOT `lib/recPolicy.ts`.** Approved. Floors are explanation-faithfulness thresholds (e.g. "don't surface a `behavioral_fit` reason if its contribution < 0.04 even though it's positive"), not branch-quota / score-cap policy. Co-locating them with the contribution model keeps explainability semantics together and prevents the recPolicy file from accreting display logic.
- **D5 — Preserve `book.reasons[]` as a derived compatibility output.** Approved. P3 introduces `ScoreContribution[]` as the new authoritative reasoning surface, but RecCard / HomeShortlist / cache-restore paths consume `book.reasons[]` directly today. P3 will compute `reasons[]` as a derived projection from `contributions[]` (top-K positive contributions formatted via the variant-pool templates) so downstream surfaces continue to work unchanged through the transition. RecCard rewires to `contributions[]` first-class in a follow-up phase, not P3A.

P3A scope when it begins: typed `ScoreContribution[]` attached during `scoreBookForUser`, `lib/explanations/compose.ts` reading them with per-kind display floors, slate-diversity guard wired into composition, explanation-faithfulness validator (`scripts/validate_explanation_faithfulness.ts`), and an end-to-end fixture replay (per phase acceptance protocol — explanations cite real positive contributions on canonical fixtures). Do **not** start P3A work outside this scope.
