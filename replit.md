...
- `lib/screenLayout.ts`: Single source of truth for top-of-screen padding. `SCREEN_TOP_PADDING = 16` and `useScreenTopPadding()` return `insets.top + 16`. Every full-screen route (Stack is `headerShown: false` in `app/_layout.tsx`) should apply this on its first child to keep onboarding, import, settings and tab routes vertically aligned.
...
- **Cover 3D treatment (single source — `CoverThumb`):** Every cover surface in the app routes through `components/CoverThumb.tsx`, so the subtle "physical book" lift lives there: a soft drop shadow (`_shadowFor(height)` — opacity 0.14, scales 0.55×–1.4× with the cover height so 24×36 thumbs and 200×300 heroes both feel right), a 1px dark spine shadow on the left edge, and a 1.5px white page-sheen on the right edge (`_PageEdgeAccents`, pure absolutely-positioned Views, no LinearGradient dep). Shadow lives on an outer wrapper View; the inner View keeps `overflow: 'hidden'` so the image stays clipped to `radius`. Pass `flat` when the cover is already inside a surface that supplies its own elevation, or when the cover is intentionally flat (e.g. faded year-stack thumbnails) — the parent shadow / image fade are otherwise unchanged. Never re-add per-call shadows on `CoverThumb` consumers; tune the lift here so all surfaces stay in lock-step.
...
- **Year-goal stack: reading is implicit:** A book the reader is actively reading is implicitly part of this year's goal — no opt-in needed. Three places enforce this rule together: (1) `app/book/[id].tsx` only renders the "Add to {year} goal stack" toggle when `localStatus === 'want_to_read'` (was `isReading || want_to_read`), so reading books never see the redundant CTA; (2) `loadYearStack(uid)` in `app/(tabs)/index.tsx` selects via `.or('status.eq.reading,and(status.eq.want_to_read,year_goal_year.eq.${currentYear})')` so the home strip surfaces every reading book regardless of `year_goal_year`; (3) the library Priority filter (`_isPriority` in `app/(tabs)/library.tsx`) and its count badge use `year_goal_year === currentYear || status === 'reading'`. The `year_goal_year` column is still set/unset normally — no auto-stamping on reading transitions — so the historical signal of "I explicitly stacked this for 2026" stays intact for want_to_read books even after they roll into reading.
...
- **Top-of-screen padding is centralized:** Expo Router's `Stack` is configured `headerShown: false` in `app/_layout.tsx`, so each route owns its own top inset. Drift had crept in (mix of `insets.top + 16`, `insets.top + 8`, hardcoded `paddingTop: 56/60`, and bare `SafeAreaView` with no extra padding — which clips on web/Android). Standardized via `lib/screenLayout.ts` (`SCREEN_TOP_PADDING = 16`, `useScreenTopPadding()` returns `insets.top + 16`). All full-screen routes (tabs, onboarding-import, import/diagnosis, import/repair-dates, import/goodreads) now use this value so the first card/headline lines up identically across the app — onboarding (account-creation flow), Goodreads import, and steady-state browsing all share the same top frame.
