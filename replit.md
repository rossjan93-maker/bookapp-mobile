...
- **Onboarding + In-App Walkthrough System (redesigned):**
    - **Architecture:** 2-phase flow. Phase 1 = brief welcome screens (`app/onboarding.tsx`, 2 slides). Phase 2 = in-app guided tour that runs inside the real app shell with a spotlight overlay, coach marks, pulsing tab-icon rings, and programmatic navigation.
    - **Phase 1 — Welcome slides** (`app/onboarding.tsx`): 2 screens only ("Welcome to readstack" + "It gets better as you read"). On completion: sets `profiles.onboarding_completed = true` + writes `readstack_walkthrough_v1 = 'home'` to AsyncStorage + writes `readstack_guided_v1 = 0` (arms rec-feed action banner) + navigates to `/`.
    - **Phase 2 — In-app walkthrough** (`lib/walkthroughEngine.ts`, `components/WalkthroughOverlay.tsx`):
        - **State machine:** `WtStep = 'home' | 'library' | 'recommend' | 'done'`. Persisted at `readstack_walkthrough_v1`. Existing users who lack the key get `'done'` (no tour).
        - **Step order:** `home` → `library` → `recommend` → `done`. `nextWtStep(current)` computes the next step.
        - **WT_DEFS:** Step definitions for `home` (tab: `/`, tabIdx: 0), `library` (tab: `/(tabs)/library`, tabIdx: 2), `recommend` (tab: `/(tabs)/search`, tabIdx: 1).
        - **WalkthroughContext** provided by `app/(tabs)/_layout.tsx`: `{ wtStep, advance, skip }`. `advance()` calls `nextWtStep`, writes to AsyncStorage, updates state, programmatically navigates to the next step's tab. `skip()` jumps to `'done'`.
        - **WalkthroughOverlay** (`components/WalkthroughOverlay.tsx`): Renders for `WT_OVERLAY_STEPS = ['home', 'library']`. Full-screen dim layer (rgba(0,0,0,0.58)) + animated coach-mark card (bottom, above tab bar) with step dots + Next/Skip + `CoachCard` fade-slide animation. `PulsingRing` at computed tab center (`SCREEN_WIDTH / TAB_COUNT * (idx + 0.5)`) with scale+opacity loop. Swipe gestures blocked during overlay steps (checked via `wtStepRef` in panResponder).
        - **Recommend step:** No overlay. When `wtStep === 'recommend'`, `app/(tabs)/search.tsx` detects it and shows `RecEntryScreen` contextually (import/intake/explore). On `onDone`: calls `advanceWt()` (→ `'done'`) + `setStep('hub')` + `loadHub()`.
        - **Analytics** (all in `lib/walkthroughEngine.ts`): `wtEvt_started`, `wtEvt_stepViewed`, `wtEvt_stepCompleted`, `wtEvt_skipped`, `wtEvt_finished`, `wtEvt_recStepReached`, `wtEvt_importCtaShown`, `wtEvt_importCtaClicked`, `wtEvt_intakeStarted`, `wtEvt_intakeSkipped`, `wtEvt_exploreClicked`.
    - **Legacy in-feed banner:** `GuidedTourContext` + `GuidedActionBanner` (inside `RecommendationsFeed`) still active. Armed with `guidedStep=0` at end of welcome slides. `advanceGuided` simplified to always jump 0→99 (old intermediate banners GuidedNotedToast/GuidedLibraryBanner removed — superseded by overlay).
    - **Routing:** `app/_layout.tsx` checks `profiles.onboarding_completed`; new users → `/onboarding`; existing users → `/`. AsyncStorage `readstack_walkthrough_v1` gates the overlay independently.
    - **Rec entry check** (`app/(tabs)/search.tsx`): `useEffect([wtStep])` — if `wtStep === 'recommend'` → show entry immediately; else: check `hasSeenRecEntry()` + `hasPersonalizationSignal()` as before. Waits for `wtStep !== null` before running.
