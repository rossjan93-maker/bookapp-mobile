...
| **BookEvidence consolidation (P4 hygiene, scheduled)** | 📋 planned · 3 batches | **Batch A** (next, smallest): retire duplicate `DARK_SIGNALS` (substring) vs `TONE_DARK_SPECIFIC` (word-boundary) drift by moving signals into `lib/evidence/signals.ts` partitioned by specific/broad, word-boundary throughout. **Batch B**: introduce typed `BookEvidence` + `deriveBookEvidence(book)` as sole classifier entry point; `getBookTraits` + evaluator both consume it; byte-identical behavior via contract validator. **Batch C** (deferred until shadow-mode calibration): add `intensity` + `emotionalWeight` dimensions + 12×6 fixture matrix incl. Emotionally Heavy Non-dark control. Full plan in chat 2026-05-18 ("8-section response"). |
