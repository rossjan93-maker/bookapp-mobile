...
**Phase 4 Analytics Surface + Evidence Tags + DNF Refinement (complete):**
- **`app/stats/index.tsx`** — Combined Reading Insights screen. Route: `/stats`. Month/Year segment tabs (no URL params). Fetches sessions from previous month → year-end in a single query. Computes `currentMonthWrap`, `prevMonthWrap`, `yearWrap` from shared session data. Month tab: 56px pages-read hero + comparison badge vs prior month + `ReadingCalendar` heat-map + `StatQuad` (days/avg/longest/streak) + top-book amber callout. Year tab: 56px books-finished hero + pages/days tagline + `YearColumns` 12-bar chart + `StatQuad` + peak-month sage callout. Sparse states handled for both tabs.
- **`ReadingCalendar`** — Reading rhythm heat-map. Grid of day dots for the month (7 columns, aligned to day-of-week). Sage green with opacity proportional to pages read; neutral border for no-read days; transparent for out-of-month cells. Width responsive via `useWindowDimensions`.
- **`YearColumns`** — 12-column bar chart. Column height proportional to reading days. Peak month label in sage, inactive months faint. Responsive width.
- **Home entry** — Single "Reading Insights" section card between Goal and Timeline. Two big numbers side-by-side (pages this month / books this year). Subline: reading days · avg pp/day. Taps to `/stats`. Gated on `pagesRead > 0 || readingDays > 0 || booksThisYear.length > 0`. Old separate monthly/yearly entry cards removed.
- **Home hierarchy**: Reading Now → Reading Goal → Reading Insights → Timeline
- **`app/wrap/month.tsx`** and **`app/wrap/year.tsx`** — retained as valid routes (editorial redesign with immersive dark headers, stat chips, rhythm calendar, column chart) but no longer linked from Home; accessible as secondary destinations.
