...
| `lib/recommender.ts` | Full recommendation engine — **dense-import mode** (`det_lanes.is_dense_import=true`): up to 3 repeated-author anchors as primary OL queries + dominant-lane OL subjects only (no literary fallback); **standard mode** fallback changed from `literary` → `general` (contemporary/popular fiction) to prevent Hemingway/Rand drift; **Step 7 lane calibration** in scoring: literary drift penalty (-0.18) when user is not a literary lane reader, noir drift penalty (-0.20) for contemporary-thriller users who get hard-boiled books, philosophy/spiritual drift penalty (-0.15); lane-aware explanations via `LANE_REASON` map replace generic trait-match text; `PD_AUTHORS` (~80 entries), `getRankedRecs()`, `getCandidateBooks()` returning `CandidateResult` with extended `RetrievalTrace` (dense_import_mode, detected_lanes, repeated_authors_used); **diversity cap**: `CACHE_VERSION='v4:'`, position-aware author cap (max 1 per author in slots 1–5, max 2 in slots 6–10), max 2 per genre |
