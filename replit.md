...
- **Onboarding + In-App Walkthrough System (fully redesigned v2):**
    - **Architecture:** 2-phase flow. Phase 1 = cinematic full-screen intro prelude (1 screen). Phase 2 = in-app guided tour inside the real app shell: spotlight aperture on live content, coach marks, pulsing tab rings, programmatic navigation.
    - **Reliability fix:** `app/_layout.tsx` exports `OnboardingBridgeContext` with a `completeOnboarding()` setter. `app/onboarding.tsx` calls it before `router.replace('/')` to update `needsOnboarding=false` in the root layout's state FIRST, preventing the routing guard from redirecting back to `/onboarding` when `segments` changes. All Supabase writes happen in the background after navigation (fire-and-forget). 8-second fail-safe timeout on the intro also guarantees forward progress.
    - **Phase 1 — Cinematic intro** (`app/onboarding.tsx`): Full-screen dark (#1c1917) composition. 3 animated book-card rectangles (different sizes/rotations/colors) assemble from below with staggered spring animations. Bold "readstack" wordmark + tagline fade in after cards settle. Single cream CTA "Get started →". Skip in top-right corner. Analytics: `intro_started`, `intro_completed`, `intro_skipped`, `intro_handoff_started` (all in `lib/onboardingAnalytics.ts`). On complete: calls `completeOnboarding()` + `router.replace('/')` immediately; then `writeWtStep('home')` + `writeGuidedStep(0)` + Supabase profile update asynchronously.
    - **Phase 2 — In-app walkthrough** (`lib/walkthroughEngine.ts`, `components/WalkthroughOverlay.tsx`):
        - **State machine:** `WtStep = 'home' | 'library' | 'recommend' | 'done'`. Persisted at `readstack_walkthrough_v1`. Existing users who lack the key get `null` (treated as `'done'` — no tour).
        - **Step order:** `home` → `library` → `recommend` → `done`.
        - **Target registry:** `registerWtTarget(key, TargetRect)` and `getWtTarget(key)` — module-level map. Screen components can register their live bounding rects. `useWalkthroughTarget(key)` hook provides `{ ref, onLayout }` for easy registration. Each step also has a `spotlightRect` fallback (fixed proportional to screen dimensions) so the overlay always has a spotlight even without live registration.
        - **WT_DEFS:** Step definitions include `spotlightRect` covering the main content area of each screen (top 42%/44% of screen height, full width) in addition to `tabIdx`, `cardPos`, `title`, `body`, `ctaLabel`.
        - **WalkthroughContext** provided by `app/(tabs)/_layout.tsx`: `{ wtStep, advance, skip }`.
        - **WalkthroughOverlay** (`components/WalkthroughOverlay.tsx`): Renders for `WT_OVERLAY_STEPS = ['home', 'library']`. **4-panel spotlight aperture** — dims the screen with 4 View panels (top/left/right/bottom) leaving a clear rectangular window over the live content area. Glow border (rgba(250,249,247,0.18), 2px) frames the aperture. Falls back to full-screen dim if no spotlight rect. `CoachCard` positions above or below the spotlight depending on spotlight vertical location. `PulsingRing` at computed tab center. Swipe gestures blocked during overlay steps.
        - **Recommend step:** No overlay. When `wtStep === 'recommend'`, `app/(tabs)/search.tsx` shows `RecEntryScreen` contextually. On `onDone`: calls `advanceWt()` → `'done'`.
        - **Analytics** (`lib/walkthroughEngine.ts`): `wtEvt_started/stepViewed/stepCompleted/skipped/finished/recStepReached/importCtaShown/importCtaClicked/importStarted/importCompleted/intakeStarted/intakeCompleted/intakeSkipped/exploreClicked`.
    - **Full analytics suite** (`lib/onboardingAnalytics.ts`): Phase 1 intro events: `intro_started/slide_viewed/completed/skipped/handoff_started/handoff_completed/handoff_failed`. Phase 2 walkthrough events. Phase 3 rec entry events.
    - **Legacy in-feed banner:** `GuidedTourContext` + `GuidedActionBanner` (inside frozen `RecommendationsFeed`) still active. Armed with `guidedStep=0`. `advanceGuided` jumps 0→99.
    - **Routing:** `app/_layout.tsx` checks `profiles.onboarding_completed`; new users → `/onboarding`; existing users → `/`. `OnboardingBridgeContext` bridges the redirect-loop gap. AsyncStorage `readstack_walkthrough_v1` gates the overlay independently.
