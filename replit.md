...
| BookEvidence Batch C (P4 hygiene) | 📋 planned | Deferred until shadow-mode calibration window opens: add `intensity` + `emotionalWeight` dimensions + 12×6 fixture matrix incl. Emotionally Heavy Non-dark control. Owner of carry-forward "emotionally heavy non-dark mismatches" concern — NOT a No-dark / P4D failure. |
...
- **BookEvidence Batch C deferred** until shadow-mode calibration window opens. Don't pre-implement `intensity` / `emotionalWeight` claims in composer copy. (Batch B shipped 2026-05-18.)
