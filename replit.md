...
- **Migrations** live in `supabase/migrations/`. Latest applied: `20260515000000_account_deletion_fix_import_rows.sql`. Pending application: `20260516000000_import_rows_user_book_id_set_null.sql` (FK promotion — see Gotchas). Apply newer files via Supabase dashboard SQL editor in filename order.
...
| **Phase 1 closeout (auth / onboarding / intake / import)** | ✅ **product accepted — 2026-05-16** | OAuth standalone-browser warm boot fixed; final-setup → quick-taste opens cleanly; quick-taste → For You handoff state-honest; mid-intake refresh resumes; Goodreads import 273-row replay clean (273 staged → 273 linked → 271 distinct user_books, two correct intra-batch duplicate collapses). Per-book imported `user_book` delete fix shipped as migration `20260516000000_import_rows_user_book_id_set_null.sql` — **pending application** via Supabase dashboard SQL editor; behaviour described in this table assumes the migration has been applied. Detail in `docs/recently_shipped.md`. |
| **P3A · contribution-grounded ranking + explanation faithfulness** | 🟡 **next active workstream — not started** | gated by Phase 1 closeout (now done). Approved decisions D1–D5 in "P3 readiness" section below. |
...
Five decisions locked before P3A implementation begins. **P3A may proceed once (a) this closeout pass is captured in docs (done), and (b) migration `20260516000000_import_rows_user_book_id_set_null.sql` has been applied** via Supabase dashboard SQL editor. The migration is a pure FK promotion (idempotent DO block) and does not require any code change to land.
...
- **`import_rows.user_book_id` FK promoted to `ON DELETE SET NULL` (migration `20260516000000`, pending application):** once applied, deleting a single imported `user_books` row will null out `import_rows.user_book_id` instead of raising `23503 import_rows_user_book_id_fkey`. The audit row is preserved (resolution / matched_book_id / raw_data intact) so "this CSV line was imported, the resulting library entry was later removed" remains queryable. **Until applied, per-book deletion of imported books still raises 23503** — code that deletes a single `user_books` row by id for an imported book should expect this and either pre-clear `import_rows.user_book_id` for that row or guard the call. The pre-existing explicit `DELETE FROM import_rows WHERE user_id = v_uid` step in `delete_own_account` / `admin_reset_account` / `reset_own_data_cold` (migration `20260515000000`, applied) is kept as defence-in-depth post-application — do **not** remove it; correctness of those functions should not depend on schema state.
