...
- **Goodreads import persistence (2026-05-15 fix):** Bulk inserts to both `books` and `user_books` are atomic per PG statement — pre-fix, a single constraint/RLS/trigger failure (or a duplicate `(user_id, book_id)` from staging collapsing two Goodreads source IDs to the same canonical books.id via shared ISBN13) silently dropped the entire 100-row chunk. `goodreadsExecutor.ts` now (a) re-keys bulk-insert results by normalised title+author rather than chunk index (RLS may filter individual rows silently → index-zip misalignment), (b) falls back per-row on bulk failure or partial result, (c) uses `.upsert(..., { onConflict: 'user_id,book_id', ignoreDuplicates: true })` for `user_books`, (d) only increments `counters.added` after the post-fetch confirms a real `user_books.id`, and (e) writes `matched_book_id` back to `import_rows` whether the link came from staging or executor recovery. Validator: `scripts/validate_goodreads_import_persistence.ts`. Live re-import smoke required before declaring product-accepted.
