...
- `lib/theme/useThemedTokens.ts`: **Bridge hook** — returns a memoized `T = { BG, INK, CARD, BORDER, DUST, STONE, FAINT, SAGE, SAGE_BG, SAGE_DEEP, SAGE_INK, AMBER }` mapping legacy token names to the active theme's semantic colors. Lets screens migrate from hex literals → themable values without rewriting `lib/tokens.ts`. Call inside any component (incl. module-scope helpers).
- `components/ThemePickerCard.tsx`: Settings → Appearance picker UI (radio-card with mini palette + sample tile per theme).
- `components/CustomTabBar.tsx`: Bottom tab bar — fully themed via `useAppTheme()` (background, accent pill, icon/label colors, badge).
...
- **Custom shelves alongside smart shelves:** `user_shelves` (named, per-user, unique-by-lower(name)) + `user_shelf_books` (CASCADE on shelf delete only — books always survive). `ShelfRow` renders both kinds identically; `activeShelf` in `library.tsx` resolves smart-first then falls back to `userShelves`. All mutations go through `lib/customShelves.ts` so duplicate handling (23505 → friendly error / idempotent add) stays consistent. Long-press a custom shelf chip → confirm-delete; long-press a library row → `ShelfPickerSheet`. Book detail also surfaces an "Add to shelf" / "On N shelves" button (gated on `userBookId`) that lazy-loads `listShelves` + `listShelfMembership` on first open and reuses the same `ShelfPickerSheet`. RLS on `user_shelf_books` requires `auth.uid() = user_id` AND `EXISTS` checks against both the referenced shelf and user_book to prevent cross-tenant attachment (broken-ownership hazard) — the denormalised `user_id` column alone isn't enough.
...
- **Theme Phase 2 — priority surfaces migrated via `useThemedTokens`:** The tab bar (`CustomTabBar`) and 7 screens (`app/(tabs)/index.tsx`, `library.tsx`, `search.tsx`, `profile.tsx`, `app/book/[id].tsx`, `app/settings.tsx`, `app/(auth)/login.tsx`) consume `useThemedTokens()` exposing `T.BG`/`T.INK`/`T.CARD`/`T.BORDER`/`T.DUST`/`T.STONE`/`T.SAGE`/`T.SAGE_BG`/`T.SAGE_DEEP` etc. so a theme change reactively re-styles the surface. Hex literals from the North palette were swapped to `T.*`; module-scope helper components each call `useThemedTokens()` themselves (out of the screen's lexical scope). **Module-scope `const` objects** (e.g. `STATUS_BADGE`, `STATUS_META`, `REC_STATUS`, `INPUT`, `CARD_STYLE`) keep literal North hex — they execute at import time before any hook can fire and are intentionally not themed yet. Components in `components/*` (RecCard, BookCard, ShelfRow, ShelfPickerSheet, RecommendBookSheet, etc.) and other screens still resolve to North via `lib/tokens.ts` until individually migrated; this is by design (additive migration) and not a regression.
