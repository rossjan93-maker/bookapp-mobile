...
| BookEvidence Batches B + C (P4 hygiene) | 📋 planned | **Batch B**: typed `BookEvidence` + `deriveBookEvidence(book)` as sole classifier entry point; `getBookTraits` + evaluator both consume it; `bookTraits.ts` TONE_DARK_SPECIFIC/BROAD migrate into `lib/evidence/signals.ts`; byte-identical behavior via contract validator. **Batch C** (deferred until shadow-mode calibration): add `intensity` + `emotionalWeight` dimensions + 12×6 fixture matrix incl. Emotionally Heavy Non-dark control. Owner of carry-forward "emotionally heavy non-dark mismatches" concern — NOT a No-dark / P4D failure. |
...
9 scripts under `scripts/`, all required green before any acceptance:
...
- **BookEvidence Batch B/C deferred** until shadow-mode calibration window opens. Don't pre-implement `intensity` / `emotionalWeight` claims in composer copy.
