...
| `lib/recPayloadCache.ts` | **Persistent rec payload cache** — AsyncStorage-backed (`@react-native-async-storage/async-storage`); key: `readstack_rec_v1_{userId}`; TTL 2h; `saveRecPayload()` / `loadRecPayload()` / `clearRecPayload()`; stores full `PersistedRecPayload` (recs, continuations, discoveries, meta, signalCount, intentTag, loadedAt); survives app restarts unlike `_recSession` (module-level, cleared on reload) |
| `lib/recPrewarm.ts` | **Background recommendation prewarm** — `triggerRecPrewarm(supabase, userId)` (fire-and-forget); module-level `_prewarmInFlight` guard prevents concurrent runs; skips if signal count < 3; calls full `getPersonalizedRecsWithExpert` pipeline, writes result to `recPayloadCache`; populates `rec_candidate_cache` (24h DB) + `_olCandidateSession` (10-min module) ahead of user navigating to Recs tab; triggered after book rated, taste tags saved, book finished (library.tsx + book/[id].tsx) |
