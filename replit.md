...
- **Genre label drift (current pre-P0A bug):** `app/edit-preferences.tsx` chip list and `lib/tasteProfile.ts:GENRE_AFFINITY_MAP` are out of sync. Six labels (`History`, `Biography`, `Business`, `Science`, `Poetry`, `Classic`) are silently dropped because they have no map entry. Plus the tier ≤ 1 gate at `lib/tasteProfile.ts:776` zeroes mapped prefs entirely for tier 2+ users. Both fixed by P0A + P1; do NOT spot-patch by adding map entries — P0A introduces the canonical taxonomy that prevents recurrence.
