...
-- Canonical pair index: least/greatest maps (A,B) and (B,A) to the same key,
-- preventing duplicate reciprocal rows regardless of who initiated the request.
...
-- Recipient inbox: filter by recipient + status, ordered by recency.
create index idx_recommendations_inbox
  on recommendations (to_user_id, status, created_at desc);

-- Sender history: all recommendations sent by a user, ordered by recency.
create index idx_recommendations_sent
...
-- Per-actor feed: a user's own events ordered by recency (covers actor_id lookups).
create index idx_activity_events_actor_feed
